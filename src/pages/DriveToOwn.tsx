...
import { Trophy, ShieldCheck, TrendingUp, Car, ArrowRight } from "lucide-react";
  ...
  return (
    <main className="container mx-auto px-4 py-8 max-w-6xl">
      <header className="mb-8">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <Link to="/drive-to-own/vs-moove">
            <Button variant="outline" size="sm">Compare with Moove <ArrowRight className="h-3.5 w-3.5 ml-1" /></Button>
          </Link>
          <Link to="/why-join">
          ...
          <CardContent>
            {loading ? (
              ...
              <div className="grid sm:grid-cols-4 gap-4">
                ...
                <Stat label="Infringements (90d)" value={eligibility.infringements_90d} negative={eligibility.infringements_90d > 0} />
                <div>
              ...
              <p className="text-sm text-muted-foreground">No reputation record yet. Complete trips to build one.</p>
            )}
          </CardContent>
        </Card>
      )}
            ...
            <Card key={p.id} className={qualifies ? "border-primary/50" : ""}>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Badge className={`uppercase ${tierColor[p.tier] || ""}`} variant="outline">{p.tier}</Badge>
                </div>
                ...
                {eligibility && (
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span>Reputation</span><span>{eligibility.reputation_score}/{p.min_reputation_score}</span>
                    </div>
                    <Progress value={Math.min(100, (eligibility.reputation_score / p.min_reputation_score) * 100)} />
                  </div>
                )}
                {qualifies && (
      ...
      <section className="mt-10">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><TrendingUp className="h-5 w-5" /> How to climb tiers</CardTitle>
          </CardHeader>
          <CardContent>
