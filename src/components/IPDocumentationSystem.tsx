...
export const IPDocumentationSystem = () => {
  ...
  const generateCIPCReport = () => {
    ...
    reportGeneration.generatePdfReport({
      title: "CIPC IP Portfolio Report",
      companyName: "MobilityOne Pty Ltd",
      companyRef: "2025/958631/07",
      sections: [
        {
          title: "IP Portfolio Summary",
          content: `Total Patents: ${patents}\nTotal Trademarks: ${trademarks}\nReady to File: ${ready}`
        },
        {
          title: "IP Assets",
          table: {
            headers: ['ID', 'Title', 'Type', 'Category', 'Priority', 'Status'],
            rows: innovations.map(i => [
              i.id,
              i.title,
              i.type.toUpperCase(),
              i.category,
        ...
        {
          title: "Detailed IP Descriptions",
          list: innovations.map(i => 
            `${i.id}: ${i.title} - ${i.description}${i.technicalDetails ? ` (Technical: ${i.technicalDetails})` : ''}${i.marketValue ? ` | Market Value: ${i.marketValue}` : ''}`
          )
        },
        {
          title: "Estimated Filing Costs",
          table: {
            headers: ['Item', 'Cost (ZAR)'],
            rows: [
      ...
      footer: "CONFIDENTIAL - MobilityOne Pty Ltd",
  ...
  return (
    <div className="space-y-6">
      ...
      <ReportShareDialog
        ...
        onOpenChange={reportGeneration.setShowShareDialog}
        title="CIPC IP Portfolio Report"
        pdfDataUrl={reportGeneration.pdfDataUrl}
        onDownload={() => reportGeneration.downloadPdf(`CIPC_IP_Report_${new Date().toISOString().split('T')[0]}.pdf`)}
        ...
        emailSubject="CIPC IP Portfolio Report - MobilityOne Pty Ltd"
        filename={`CIPC_IP_Report_${new Date().toISOString().split('T')[0]}.pdf`}
      />
    </div>
  );
};